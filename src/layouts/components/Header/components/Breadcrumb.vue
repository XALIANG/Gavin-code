<template>
    <el-breadcrumb :separator-icon="ArrowRight">
        <transition-group
            name="breadcrumb" mode="out-in"
            tag="div"
        >
            <template v-if="breadcrumbList">
                <!-- 首页面包屑不要可以直接删除 -->
                <el-breadcrumb-item
                    v-if="breadcrumbList[0].meta.title !== '首页'" :key="HOME_URL"
                    :to="{path: HOME_URL}"
                >
                    <div class="breadcrumb-item">
                        <el-icon v-if="themeConfig.breadcrumbIcon" class="breadcrumb-icon">
                            <HomeFilled/>
                        </el-icon>
                ...
                <el-breadcrumb-item v-for="(item, index) in breadcrumbList" :key="item.path">
                    <div class="breadcrumb-item el-breadcrumb__inner is-link" @click="onBreadcrumbClick(item, index)">
                        <el-icon v-if="item.meta.icon && themeConfig.breadcrumbIcon" class="breadcrumb-icon">
